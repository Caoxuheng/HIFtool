...
def evalu_model_specific(model,opt,model_folder,test_epoch,end_epoch,dataset_name):
    ...
    R = torch.tensor(np.load(opt.srfpath)).float()
    if dataset_name.lower()=='cave':
        lst =[1,2,7,8,9,10,11,15,19,23,26,27]
        size = 512
    elif dataset_name.lower() =='harvard':
    ...
    for ax, idx in enumerate(lst):
        ...
        Spatialdown = SpaDown(opt.sf)
        LRHSI, HRMSI, GT_ = getInputImgs(opt,dataset_name, idx, Spatialdown)
        GT = torch.FloatTensor(GT_).T.unsqueeze(0)
        LRHSI,HRMSI = LRHSI, HRMSI
        optimizer = torch.optim.Adam(model.parameters(),lr = 3e-5)
        PixelLoss=nn.L1Loss()
        Spatialdown = Spatialdown
        LR,HR,GTs=[],[],[]
        ...
        for epoch in range(end_epoch):
            for b in range(2):
                ...
                Predlr = rearrange(Predlr,' s (b c) h w -> (s b) c h w ',c=LR.shape[1])
                Prehr = SpeDown(Pred,R)
                loss = PixelLoss(Predlr,LR)+PixelLoss(Prehr,HR)
                loss.backward()
                optimizer.step()
            if epoch%10==0:
                model.eval()
